# Welcome to the Granger Causality in Extremes Implementation!
# This file contains the two main R functions from our framework.
...
Extreme_causality_test <- function(x, y, z = NULL, lag_future = 1, lag_past = 0, nu_x = 0.3, q_y = 0.2, q_z = 0.1, both_tails = TRUE, instant = FALSE, p_value_computation = FALSE, bootstrap_repetitions = 50, choice_of_F = 0.5) {
  ...
  F_u <- function(imput, output) { # This is the F^{truc}_Y(t) function that we opt for
    ifelse(output < quantile(imput, choice_of_F), 0, (ecdf(imput)(output)))
  }
  ...
  if (p_value_computation == FALSE) {
    if (CTC <= (1 + baseline) / 2) {
      return(data.frame(output = "No causality", CTC = CTC, baseline = baseline))
    }
    if (CTC > (1 + baseline) / 2) {
      return(data.frame(output = "Evidence of causality", CTC = CTC, baseline = baseline))
    }
  }
...
# TODO: un-nest other functions (F_u, CTC_baseline, CTC_masked). Though, they are not self-contained, which is a problem and requires some rewrite.
