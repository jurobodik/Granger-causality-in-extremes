...
meteo_stations_data <- read.csv(file = paste0(data_folder, "meteo_stations_data.txt")) # Other measurments such as temperature or humidity. These measurments were not from 'AIR' station but at the nearby 'LUZ' station, so there is a small data-collection discrepacy
...
read_station_precip_causes_discharge <- function(station, minimum_number_of_observation = 5000) {
  z1 <- discharges[, c(1, station + 1)]
  z1$Date <- as.Date(z1$Date)
  z2 <- precipitation[, c(1, 3)]
